...
特定の処理をして停止するコンテナを動かしてみましたが、停止してしまっているので今ひとつピンと来ないかもしれません。続いて、稼働していることがわかるコンテナを利用しまう。静的コンテンツのWebsiteを提供するコンテナを起動してみます。Websiteなのでhttpdという文字列でコンテナレジストリを検索します。
